...
pub fn create_transaction_p2wpkh_final(data: serde_json::Value) -> String {
    ...
    for input in data["vin"].as_array().unwrap() {
        let witness = input["witness"].as_array().unwrap();
        let witness_count = format!("{:02x}", witness.len());
        raw_transaction += &hex_to_little_endian(&witness_count);
        for w in witness {
            let w_length = format!("{:02x}", w.as_str().unwrap().len() / 2);
            raw_transaction += &hex_to_little_endian(&w_length);
            raw_transaction += w.as_str().unwrap();
        }
    }
